...
#
# This helper defines a function to search for Swift files that reference
# a given type name. It returns a temporary file containing a list of matching files.
#
# Usage (when sourcing):
...
find_referencing_files() {
    ...
    # Search for occurrences of the type name as a whole word in Swift files,
    # excluding common build directories.
    grep -rlE "\\b$type_name\\b" --include "*.swift" "$search_root" \
