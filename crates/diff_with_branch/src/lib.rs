...
mod tests {
    ...
    #[test]
    fn test_diff_against_explicit_branch() {
        let dir = tempdir().expect("Failed to create temp dir");
        let temp_path = dir.path();
        init_git_repo(temp_path);

        let file_path = temp_path.join("branchtest.txt");
        {
            let mut file = File::create(&file_path).expect("Failed to create file");
            writeln!(file, "Initial content").expect("Failed to write to file");
        }
        Command::new("git")
            .args(["add", "branchtest.txt"])
            .current_dir(temp_path)
            .output()
            .expect("Failed to add file");
        Command::new("git")
        ...
        let result = run_diff_against(&file_path, "old-branch");
        assert!(result.is_ok());
