...
fn main() -> Result<()> {
    // Parse command-line arguments using Clap with proper flag actions.
    ...
    let slim = matches.get_flag("slim");
    let singular = matches.get_flag("singular");
    let force_global = matches.get_flag("force_global");
    let include_references = matches.get_flag("include_references");
    ...
    let file_path = run_command(&["find_prompt_instruction", &git_root], None)
        ...
        .to_string();
    if verbose {
        println!("Found TODO file: {}", file_path);
    }
    ...
    if singular {
        ...
        found_files_path = {
            ...
            writeln!(temp, "{}", file_path).context("Failed to write TODO file in singular mode")?;
            temp.into_temp_path()
                .keep()
        ...
        if verbose {
            println!("Types file generated: {}", types_file.trim());
        }
        let def_files_content = run_command(
            &["find_definition_files", types_file.trim(), &search_root],
            None,
        )
        ...
        let mut found_files = std::fs::read_to_string(&found_files_path)
        ...
        if slim {
            ...
            std::fs::write(&found_files_path, found_files.trim())
                .context("Failed to write filtered list")?;
        }
        // Apply file exclusions.
        if !excludes.is_empty() {
            if verbose {
                println!("Excluding files: {:?}", excludes);
            }
    ...
    // Optionally print final list if verbose.
    if verbose {
        println!("Final list of files:");
        let files_content = std::fs::read_to_string(&found_files_path)
            .context("Failed to read found files list")?;
    ...
    if include_references {
        println!("Warning: The --include-references option is experimental.");
    }
    println!("--------------------------------------------------\n");
